...
extension View {
    ...
    func navigation<Item: Identifiable, Destination: View>(item: Binding<Item?>, destination: @escaping (Item) -> Destination) -> some View {
        let isActive = Binding<Bool> {
        ...
        return self.modifier(NavigationViewModifier(isActive: isActive, destinationView: destination(item.wrappedValue!)))
    }
}
